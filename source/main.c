...
void loadNro()
{
    ...
    if (strlen(g_nextNroPath) == 0) //If the NRO didn't use envSetNextLoad
    {
        strcpy(g_nextNroPath, "sdmc:/switch/Goldleaf.nro"); //Load Goldleaf because it looks different than hbmenu and can still launch NROs
        strcpy(g_nextArgv,    "sdmc:/switch/Goldleaf.nro");
    }
